...
class TestFindSpikeLocations:
    def test_finds_peaks_in_sinusoid(self):
        ...
        locs = find_spike_locations(signal, peak_threshold=5.0, fs=fs, spike_template_width=51)
    ...
    def test_excludes_peaks_near_edges(self):
        ...
        signal[500] = 100.0
        locs = find_spike_locations(signal, peak_threshold=5.0, fs=fs, spike_template_width=stw)
    ...
    def test_returns_empty_for_flat_signal(self):
        ...
        locs = find_spike_locations(signal, peak_threshold=5.0, fs=10000, spike_template_width=51)
        assert len(locs) == 0
    ...
    def test_respects_peak_threshold(self):
        ...
        locs = find_spike_locations(signal, peak_threshold=10.0, fs=10000, spike_template_width=51)
        assert len(locs) == 1
        assert locs[0] == 5000
